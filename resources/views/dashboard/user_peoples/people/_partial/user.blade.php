<div class="cli_hot_user_Flex">
    <div class="cli_hot_user_left">
        <div class="blockForm">

            <div class="image-upload__cabinet ">
                @if(isset($item->avatar))
                    <a class="co"
                       href="{{ asset(Storage::disk('public')->url($item->avatar)) }}"
                       data-fancybox="avatar">

                        <div class="site_avatar"
                             style="background-image: url('@if($item->avatar){{  asset(intervention('120x120', $item->avatar, 'users/' . $item->id  . '/avatar')) }} @endif '); width: 100px; height: 100px"></div>

                    </a>
                @else
                    <div class="site_avatar"
                         style="background-image: url('{!! 'data:image/svg+xml;base64,' !!}'); width: 100px; height: 100px"></div>
                @endif
            </div>

        </div><!--.blockForm-->
    </div><!--.cli_hot_user_left-->
    <div class="cli_hot_user_right">
        <div class="hot_user_username">{{(isset($item->name))? $item->name : ' - '}}</div>
        <div class="hot_user_email">{{(isset($item->email))? $item->email : ' - '}}</div>
        @if(isset($item->phone))
            <div class="hot_user_phone"><a href="{{ $item->phone }}">{{ format_phone($item->phone)  }}</a></div>
        @endif
    </div><!--.cli_hot_user_right-->
</div>
